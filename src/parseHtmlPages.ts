...
(async () => {
    ...
    for (let spell of allSpells) {
        ...
        const data: Record<string, string | undefined> = {
            ...Object.fromEntries(Object.entries(infoToParse).map(([key, searchStr]) => {
                ...
                if (searchStr === "<strong>Источник:</strong> ") {
                    ...
                    const secondAttempt = infoLines.find(l => l.startsWith(alternativeSpelling));
                    if (secondAttempt) {
        ...
        if (data.source) {
            data.source = data.source.replaceAll("<span>", "").replaceAll("</span>", "");
        }

        (function reportOnStrangeLines() {
            const unknownLines = infoLines.splice(1).filter(l => !knownLines.some(kl => l.startsWith(kl))).filter(x => !x.includes("<div itemprop=\"description\">"));
            if (unknownLines.length > 0) {
                console.log(`Unknown lines found in ${spell.slug}: ${unknownLines}`)
            }
        })();

        await fs.writeFile(writeFilePath, JSON.stringify({...spell, ...data}, null, 2));
    }
})();
